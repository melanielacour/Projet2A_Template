...
class UserMovieService:
    ...
    def get_watchlist(self, id_user):
        """
        Méthode qui récupère la liste des films que l'utilisateur souhaite regarder.
        """
        return self.user_movie_dao.get_movies_by_user(id_user, status="to_watch")

    def get_seenlist(self, id_user):
        """
        Méthode qui récupère la liste des films que l'utilisateur a déjà regardés.
        """
    ...
    def add_movie_to_list(self, id_user: int, id_film: int, status: str):
        """
        Méthode qui ajoute un film dans la liste de l'utilisateur ou modifie son statut
