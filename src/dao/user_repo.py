...
class UserRepo:
    """
    Cette méthode permet de récupérer un utilisateur par ID ou pseudo,
    d'ajouter de nouveaux utilisateurs, de mettre à jour leur pseudo,
    mot de passe ou statut d'éclaireur.
    """
    ...
    def get_by_id(self, user_id: int) -> Optional[User]:
        """
        Cette méthode permet de récupèrer un utilisateur
    ...
    def get_by_username(self, username: str) -> Optional[User]:
        """
        Cette méthode permet de récupèrer un
        utilisateur à partir de son pseudo.
    ...
    def insert_into_db(
            self, username: str, salt: str, hashed_password: str) -> User:
        """
        Cette méthode permet d'insèrer un nouvel utilisateur avec un pseudo,
        un sel et un mot de passe haché.
        ...
        ---------
        User
            Un objet User représentant l'utilisateur créé
    ...
    def update_pseudo(self, user_id: int, new_pseudo: str) -> bool:
        ...
        le pseudo d'un utilisateur dans la base de données.
    ...
    def update_password(self, user_id: int, hashed_password: str) -> bool:
        """
        Cette méthode permet de mettre à jour le mot de passe haché
        ...
        query = "UPDATE users SET password = %(hashed_password)s WHERE id = %(user_id)s"
        with self.db_connection.connection() as connection:
            with connection.cursor() as cursor:
                cursor.execute(
                    query, {
    ...
    def update_status(self, user_id: int, is_scout: bool) -> bool:
        """
        Cette méthode permet de mettre à jour le statut d'éclaireur
        (is_scout) d'un utilisateur dans la base de données.
        ...
        ---------
        Bool
            True si le statut de l'utilisateur a été mis
            à jour avec succès, sinon False.
        """
    ...
    def delete_by_id(self, user_id: int) -> bool:
        """
        Cette méthode permet de supprimer un
        utilisateur de la base de données par son identifiant.
