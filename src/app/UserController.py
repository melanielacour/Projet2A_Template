...
from src.app.init_app import user_repo, user_service
...
def register_user(pseudo: str, password: SecretStr):
    try:
        message = user_service.register_user(
            pseudo, password.get_secret_value()
            )
...
async def update_pseudo(
    request: UpdatePseudoRequest, user_id: int = Depends(get_current_user)
):
    try:
        message = user_service.update_pseudo(
            user_id=user_id, new_pseudo=request.new_pseudo
        )
        return {"message": message}
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
async def update_password(
    request: UpdatePasswordRequest, user_id: int = Depends(get_current_user)
):
    try:
        message = user_service.update_password(
            user_id=user_id,
...
async def get_user_by_user_pseudo(
    pseudo: str, user_id2: int = Depends(get_current_user)
):
...
async def get_user_by_user_id(
    id: int, user_id2: int = Depends(get_current_user)
):
